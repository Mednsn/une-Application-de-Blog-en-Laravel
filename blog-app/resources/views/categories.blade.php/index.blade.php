...
<body>
    ...
    <a href="{{ route('catrgories.create') }}" class="btn btn-primary">Nouvelle Tâche</a>

    @foreach($categories as $category)
        <div class="category {{ $category->completed ? 'completed' : '' }}">
            <h3>{{ $category->title }}</h3>
            <p>{{ $category->description }}</p>
            <p>Statut: {{ $category->completed ? 'Terminée' : 'En cours' }}</p>
